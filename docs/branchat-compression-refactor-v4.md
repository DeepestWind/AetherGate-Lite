...
**role 字段保持现有超集定义**，新压缩流程不再向原始树写入 summary 节点，这是行为约束而非类型约束，不缩窄 role 的允许值范围（存量数据中可能已存在旧压缩产生的 summary/system/tool 节点）：

```python
class ChatMessageRecord:
    id: str
    role: str            # 保持现有超集："user" | "assistant" | "summary" | "system" | "tool"
                         # 新压缩流程不再新增树内 summary 节点，但不改变类型定义
    content: str
    parent_id: str | None
...
`kind="node"` 时前端 adapter 从 `message_nodes[source_node_id]` 补齐展示字段（`timestamp / pinned / modifiedFrom / callInfo / errorMessage / status` 等），`kind="summary"` 时只使用最小字段：

```python
class VisibleMessageResponse:
    virtual_id: str             # 稳定 id，规则见下方说明
...
- `kind="node"`：`virtual_id = source_node_id`（与节点 id 保持一致，天然稳定）
- `kind="summary"`：`virtual_id = f"summary:{branch_id}:{head_message_id}:{summary_index}"`

`summary_index` 为该 summary 在 `compressed_path` 中的位置索引（从 0 起），确保同一分支同一状态下 id 不变，避免 React key 变化引发重渲染、展开状态重置、滚动抖动。
...
当前库中已存在旧压缩机制产生的数据（改过 `parent_id`、插入了树内 summary 节点、写入了 `archived`）。v4 采用**新写入走新机制，存量数据保持兼容**的策略，具体规则如下：

**存量数据不做修复**，旧压缩产生的数据原样保留，原因：
- 修复存量数据风险高，可能破坏现有对话的完整性
- 旧数据的 `compressed_at_head_message_id` 为 NULL，`build_context_view` 会直接走原始链路，旧的压缩 summary 节点会出现在路径里

**`build_context_view` 需要能识别旧格式数据**，对原始链路中遇到的旧 summary 节点（`role="summary"` 且在 `node_store` 中）做角色映射处理（映射为 `system`），不报错：
...
def _walk_to_dict(node_id: str) -> list[dict]:
    ...
    while current:
        # 兼容旧压缩产生的树内 summary 节点
        role = current.role
        if role == "summary":
            role = "system"
        path.append({"role": role, "content": current.content})
        current = node_store.get(current.parent_id)
    return list(reversed(path))
```

**新机制只对新产生的压缩操作生效**，旧对话如果触发了新的压缩，会生成新格式的 `compressed_path_json` 缓存，从此走新路径。

**后续清理时机**：待新机制稳定运行一段时间后，可对存量数据做一次性修复脚本，将旧 summary 节点和 `archived` 标记清理干净，再删除 `archived` 字段。
...
### 新增：build_context_view（供 LLM API 调用使用）

**注意：summary 条目映射为 `system` 角色发给模型**，因为模型 API 只接受 `system | user | assistant | tool`（`chat.py line 13`），沿用现有代码的处理方式（`chat_sessions.py line 666`）：
...
def build_context_view(branch: ChatBranch) -> list[dict]:
    ...
    return [
        {
            "role": "system" if e["role"] == "summary" else e["role"],
            "content": e["content"]
        }
        for e in cached_path
    ]
...
def _walk_to_dict(node_id: str) -> list[dict]:
    ...
    while current:
        role = current.role
        if role == "summary":
            role = "system"  # 兼容旧压缩产生的树内 summary 节点
        path.append({"role": role, "content": current.content})
        current = node_store.get(current.parent_id)
    return list(reversed(path))
...
def flatten_visible_messages(
    ...
    kind="node" 时前端 adapter 从 message_nodes[source_node_id] 补齐展示字段。
    """
    ...
    summary_index = 0
    for entry in cached_path:
        if entry["role"] == "summary" or not entry["node_id"]:
            result.append(VisibleMessageResponse(
                virtual_id=f"summary:{branch_id}:{branch.head_message_id}:{summary_index}",
                kind="summary",
                role="summary",
...
**产品决策声明：优先压缩路径中最早出现的一段连续非 pinned 区间，不跨 pinned 节点，不做全局 token 最优选段。** 这是有意为之的简单策略，不要误解为"自动挑 token 最重的区间"去优化。

**关键点：始终基于 `full_path` 原顺序工作，在原路径上找连续可压缩区间，用 summary 原位替换，不拆组重拼。**
...
def compress(branch: ChatBranch, tier: Tier):
    ...
    # 构建缓存路径：在原始顺序上，压缩段替换为 summary 条目，其余原样
    cached_path = []
...
| 视图 | 数据来源 | 内容 | 展示字段来源 |
|------|---------|------|------------|
| 主消息区 | `response.visible_messages` | 含 summary 条目的上下文视图 | `kind="node"` 时从 `message_nodes[source_node_id]` 补齐 |
| 右侧树面板 | `response.message_nodes` | 完整原始树，不含 summary | 不变 |

前端改动范围：`chat-adapters.ts line 275/320` 附近，主消息流改为优先读取 `response.visible_messages`，adapter 层在 `kind="node"` 时从 `message_nodes` 按 `source_node_id` 查找并补齐 `timestamp / pinned / modifiedFrom / callInfo / errorMessage / status` 等展示字段。
...
- 新增 `build_context_view(branch)`，注意 summary → system 角色映射，兼容旧树内 summary 节点
- 新增 `flatten_visible_messages(branch, branch_id)`
- 新增 `prepare_context(branch)`
- 新增新版 `compress(branch, tier)`，替换旧压缩函数调用入口
...
- adapter 层补齐 `kind="node"` 的展示字段
- 树面板保持读取 `message_nodes`，不改动
- 确认 summary 条目正确渲染，virtual_id 稳定不抖动

**Step 6：接入 version 递增**
- 编辑节点内容时调用 `update_node_content`
- pin/unpin 操作时调用 `update_node_pinned`
- 验证缓存失效和回退链路正确

> append 优化和 `archived` 字段清理留待后续版本。存量数据无需一次性修复，旧格式由 `_walk_to_dict` 兼容处理。
...
| 路径中存在 pinned 节点 | 压缩区间在原始顺序上连续选取，pinned 节点不被纳入，前后节点顺序不变 |
...
| pin/unpin 已被压缩段内的节点 | version 递增，`_sources_valid` 失败，缓存失效 |
| pin/unpin survivor 区节点 | version 递增，`_survivors_valid` 失败，缓存失效 |
| main 再次触发压缩 | 重新生成缓存，覆盖旧缓存 |
| summary 条目在主消息区显示 | `visible_messages` 中 kind="summary" 条目正常渲染，virtual_id 稳定 |
| 切换分支后 summary virtual_id | head_message_id 变化，新 virtual_id 与旧分支不冲突 |
| 树面板展示原始树 | 继续基于 `message_nodes`，不含 summary，结构完整 |
| `flatten_messages()` 原有调用点 | 行为不变，返回原始节点链路 |
| 旧压缩数据（存量） | `build_context_view` 走原始链路，旧 summary 节点映射为 system，不报错 |
| 旧对话触发新压缩 | 生成新格式缓存，后续走新路径，旧数据不受影响 |
| summary 发给模型的 role | 缓存路径和原始链路均映射为 system，不出现 role="summary" |
